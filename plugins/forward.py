...
async def forward_message(event):
    try:
        ...
        if updated_message:
            ...
            await app.send_message(destination_channel_id, updated_message, link_preview=False)
...
async def forward_message(event):
    try:
        ...
        # Retrieve the destination channel based on the source channel from the database
        channel_data = await u_db.channels_tb.find_one({"source_channel": source_channel_id})
        ...
        # Fetch replacement data from the ReplacementData collection for the source channel
        replacement_log = await u_db.replacement_data_tb.find_one({"source_channel": source_channel_id})
